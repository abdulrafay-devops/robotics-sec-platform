...
#
# Runs the full DevSecOps pipeline against the workspace source tree
# in unit-style mode (Gate 6 acceptance is skipped because the gate is
# itself the acceptance run; we do not want a recursion).
#
...
# Sync live code/config from the host volume to ensure latest gate inputs are run.
if [[ -d /vagrant/vm-ai/devsecops ]]; then
    say "Syncing live pipeline code/config from /vagrant/vm-ai/devsecops to /opt/lab/vm-ai/devsecops"
    cp -f /vagrant/vm-ai/devsecops/*.py /vagrant/vm-ai/devsecops/*.sh /vagrant/vm-ai/devsecops/*.yml /opt/lab/vm-ai/devsecops/
    chmod +x /opt/lab/vm-ai/devsecops/*.sh /opt/lab/vm-ai/devsecops/*.py
fi

say "=== Stage 5 pipeline (unit mode: Gate 6 skipped) ==="
LAB_SKIP_ACCEPTANCE=1 LAB_PIPELINE_PY="${PY}" LAB_SOURCE_DIR=/vagrant bash "${RUN}" 2>&1 | tee -a "${LOG}"
RC=${PIPESTATUS[0]}
